...
# Written by Tianpeng Wang, Modified from Joana Meier.
...
awk -F"\t" 'BEGIN{scaff="";add=0}{
        ...
        lastPos=pos
}' ${file}.map  | perl -p -ne 's/scaffoldA?C?0?//' > better.map
...
echo "genotypeoutname: ${file}.geno" >> par.PED.EIGENSTRAT.${file}
